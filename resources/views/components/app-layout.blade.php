                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @if(auth()->user()->unreadNotifications->count() > 0)
                                    <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">
                            ...
                            >
                                ...
                                <div class="max-h-96 overflow-y-auto">
                                    @forelse(auth()->user()->notifications()->take(10)->get() as $notification)
                                        <a
                                            href="{{ route('incidents.show', $notification->data['incident_id'] ?? 0) }}"
                                            class="block px-4 py-3 border-b hover:bg-gray-50 {{ $notification->read_at ? '' : 'bg-green-50' }}"
                                        >
                                            <div class="flex gap-3">
                                                <div class="text-lg">
                                                    @if(($notification->data['type'] ?? '') === 'incident_assigned')
                                                        👷
                                                    @elseif(($notification->data['type'] ?? '') === 'incident_status_changed')
                                                        🔄
                                                    @else
                                                        💬
                                                    @endif
